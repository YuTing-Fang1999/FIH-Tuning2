...
class ROI_Setting_Widget(QtWidgets.QWidget):
    to_setting_signal = pyqtSignal(list, list, list)

    def __init__(self):
        super(ROI_Setting_Widget, self).__init__()
    ...
    def set_roi_coordinate(self, img_idx, img, roi_coordinate, filename):
        ...
        self.setting.params["roi"][img_idx-1]=[x, y, w, h]
        self.draw_ROI()
        

    def draw_ROI(self):
        img_select = self.img.copy()
        print('draw_ROI', self.setting.params["roi"])
        for i, roi in enumerate(self.setting.params["roi"]):
    ...
    def add_ROI_item(self):
        ...
        self.setting.params["roi"].append([])
    ...
    def set_param(self):
        self.setting.params["target_type"] = []
        self.setting.params["target_score"] = []
        self.setting.params["target_weight"] = []

        for i, roi in enumerate(self.setting.params["roi"]):
            if len(roi)==0: continue
            self.setting.params["target_type"].append(self.type_selector[i].type)
            self.setting.params["target_score"].append(float(self.target_score[i].text()))
            self.setting.params["target_weight"].append(float(self.target_weight[i].text()))

        # print(self.setting.params["target_type"])
        # print(self.setting.params["target_score"])
        # print(self.setting.params["target_weight"])

    def set_UI(self):
        self.add_title()
        self.set_photo()

        # if 'target_type' in self.setting.params and len(self.setting.params["target_type"])>0:
        #     for i in range(len(self.setting.params["target_type"])):
        #         self.add_ROI_item()
        #         self.type_selector[i].setCurrentText(self.setting.params["target_type"][i])
        #         self.target_score[i].setText(str(self.setting.params["target_score"][i]))
        #         self.target_weight[i].setText(str(self.setting.params["target_weight"][i]))


if __name__ == '__main__':
    import sys
    app = QtWidgets.QApplication(sys.argv)
    window = ROI_Setting_Widget()
